...
with this_week as (SELECT (sum(duration_in_s))/count(distinct blended_user_id) as lw_avg_session_duration_s FROM `ra-development.analytics.segment_web_sessions__stitched`
...
and first_page_url_host = 'rittmananalytics.com'),
     prev_week as (SELECT (sum(duration_in_s))/count(distinct blended_user_id) as pw_avg_session_duration_s FROM `ra-development.analytics.segment_web_sessions__stitched`
