...
FROM {{ ref('customer_master') }} AS customer_master
    ...
    LEFT JOIN {{ ref('client_invoices') }} AS harvest_invoices ON customer_master.harvest_customer_id = harvest_invoices.client_id AND harvest_projects.id = harvest_invoices.project_id
