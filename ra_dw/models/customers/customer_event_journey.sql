        ...
        SELECT *,
         ...
         FROM (
              SELECT customer_id,
                     ...
                     event_ts,
                     LAG(case when event_type like '%Email%' then 'Presales' when event_type like '%Bill%' then 'Delivery' else event_type end,1) OVER (PARTITION BY customer_id ORDER BY event_ts) AS last_event
